...
unsigned char Distance[] = "000.0cm";
...
int CSB()
{
   ...
   k = (TH0*256+TL0);			   //低八位TL0记满溢出，给高八位TH0加1，没低八位加256次溢出一次给高八位加1
   t =k*(12/22118400.0);
...
//   write_data (s%10+48);
  	Distance[0] = s/100 + '0' ;
	Distance[1] = s/10%10 + '0' ;
	Distance[2] = s/10%10 + '0' ;

	show_Typeface(2,1,Distance);  //只调用超声波时去掉
